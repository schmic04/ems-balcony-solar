...
def configure_time_range(start_date=None, end_date=None, start_time=None, end_time=None):
    ...
    # Debug-Ausgabe
    print("   ⚙️ Zeitbereich konfiguriert:")
    ...
    if end_datetime:
        ...
        print("      🔴 Ende:  Offenes Ende (alle verfügbaren Daten)")
...
def find_optimal_start_timestamp(timestamps, datetime_list, start_datetime):
    ...
    print("   🔍 Suche vom neuesten Zeitstempel rückwärts...")
    ...
    for i in range(len(timestamps) - 1, -1, -1):
        ...
        if timestamp_date < start_date:
            print("   ✅ Optimaler Startzeitstempel gefunden!")
        ...
        if timestamp_date == start_date:
            print("   ✅ Exakte Übereinstimmung gefunden!")
            print(f"   📍 Index: {i} (Zeitstempel: {timestamps[i]})")
            print(f"   📅 Datum: {timestamp_date} = {start_date}")
            return i
    ...
    print("   💡 Verwende ältesten verfügbaren Zeitstempel (Index 0)")
    return 0
        ...
        try:
            ...
            if timestamp_points:
                ...
                if new_points:
                    ...
                    print("   ℹ️ Daten bereits vorhanden (Duplikate)")
            else:
                print("   ❌ Keine Daten im Zeitbereich")

        except Exception as e:
            print(f"   ⚠️ Fehler: {e!s}")
            continue
